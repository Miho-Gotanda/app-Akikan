...
public class HardAkikanController : MonoBehaviour
{
    ...
    private HardCount hardCount;
   
    // Start is called before the first frame update
    void Start()
    {
        hardCount = GameObject.Find("DastBox").GetComponent<HardCount>();
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        else if(collision.gameObject.tag == "Earth")
        {
            ...
            else if(gameManager.damage!=0&&hardCount.counter!=0)
            {
                Instantiate(akikanPrefab, Position.transform.position, Quaternion.identity);
                Destroy(this.gameObject);
            }
